"""
AI QA Demo: evaluate HR-bot answers via DeepEval (Faithfulness, Answer Relevancy, etc.).
Uses OpenRouter as the judge LLM for metrics.
"""
import os
import csv
import json
...
# Judge LLM: DeepEval calls this to score answers (relevancy, faithfulness, etc.)
class OpenRouterLLM(DeepEvalBaseLLM):
    """Custom LLM for DeepEval via OpenRouter (LangChain)."""
    def __init__(self):
        api_key = (os.environ.get("OPENROUTER_KEY") or "").strip()
        # Common typo: extra leading letter (e.g. ssk-or instead of sk-or)
    ...
    def generate(self, prompt: str, **kwargs):
        # DeepEval passes schema=... and expects a schema object (e.g. .claims), not raw text
        ...
        if schema is not None:
            # Strip markdown code blocks if present, then parse JSON into schema object
            text = (out or "").strip()
            for marker in ("```json", "```"):
                if marker in text:
            ...
            try:
                ...
                # Fallback: build schema from dict with safe defaults when model returns wrong shape
    ...
    async def a_generate(self, prompt: str, **kwargs):
        ...
        schema = kwargs.get("schema")
        if schema is not None:
            # Same JSON/schema parsing as generate() (strip markdown, parse, fallback)
            text = (out or "").strip()
            for marker in ("```json", "```"):
                if marker in text:
                    idx = text.find(marker)
...
# Load knowledge base used as retrieval_context for metrics
def load_document() -> str:
    with open(DOCUMENT_PATH, "r", encoding="utf-8") as f:
        return f.read().strip()


# Run metric.measure(); on exception return (score=None, reason='N/A') so other metrics still run
...
# Read dataset.csv (delimiter ;), return list of (LLMTestCase, category)
...
def main():
    api_key = (os.environ.get("OPENROUTER_KEY") or "").strip()
    if not api_key or "REPLACE_ME" in api_key or "YOUR_KEY" in api_key:
        print("⚠️  Set OPENROUTER_KEY in .env (get key from openrouter.ai)")
        return

    # Load knowledge base and test cases from CSV
    try:
    ...
    # DeepEval metrics: each calls the judge LLM to score one aspect (relevancy, faithfulness, etc.)
    ...
    # Prepare report file (one CSV per run)
    ...
    # Run all metrics per test case and collect scores/reasons
    for i, (test_case, category) in enumerate(test_data, 1):
        ...
        # Pass/fail: higher is better for relevancy/faithfulness/contextual_recall; lower for toxicity/bias
        ...
        # Classify failure type and severity for reporting
        if overall_pass:
            failure_type = "passed"
        ...
        # Append row for CSV (column order is set in fieldnames below)
        report_rows.append({
            "run_date": run_date,
            "run_time": run_time,
    ...
    # CSV column order: result first (test_id, pass/fail, scores), then context, then metadata and reasons
    fieldnames = [
